			...
			<form action="${context}" method="GET">
				<table class="form">
					<tbody>
						...
						<tr>
							<td>Food or Beverage</td>
							<td>	
								<select name="activity">
									<c:forEach var="a" items="${allFbs}">
										<c:if test="${ao != null}">
											<option ${a.id == ao.fb.id ? 'selected' : ''} value="${a.id}">${a.name}</option>
										</c:if>
										<c:if test="${ao == null}">
						...
						<tr>
							<td>Sell Time</td><td><input class="datepicker" type="text" name="sstime" value="<fmt:formatDate pattern="MM/dd/yyyy" value="${ao.sellTime}" />"/></td>
						</tr>
						<tr>
